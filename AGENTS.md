...
npm run build        # Static export to /out — always verify changes build cleanly
...
- **Static export**: `output: 'export'` in next.config — no server-side features (no API routes, no `getServerSideProps`, no dynamic headers/cookies). Every route must work with `generateStaticParams()`. If a build fails with "Page with dynamic = error couldn't be rendered statically", you're using a server-only feature.
