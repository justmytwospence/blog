...
const PROJECT_ACTIVITY_PATH = path.join(process.cwd(), 'data', 'project-activity.json');

/**
 * Map of project slug -> last-updated date (ISO), read from the committed
 * activity snapshot (data/project-activity.json, regenerated by
 * scripts/sync-project-activity.mjs). Used to order projects by recency.
 */
function getProjectUpdatedMap(): Record<string, string> {
  try {
    const raw = fs.readFileSync(PROJECT_ACTIVITY_PATH, 'utf8');
    const data = JSON.parse(raw) as { updated?: Record<string, string> };
    return data.updated ?? {};
  } catch {
    return {};
  }
...
export function getAllProjects(): Project[] {
  ...
  const updated = getProjectUpdatedMap();
